...
<div class="@if (session()->has('message')) !hidden @endif">
	<form id="group-reg-form" action="{{ route('groups.store') }}" class="grid mobile:grid-cols-1 tablet:grid-cols-2 laptop:grid-cols-6 gap-6" method="post" enctype="multipart/form-data" novalidate>
		@csrf
		@method('post')
		...
		<x-fields::input class="col-span-3" id="fname" type="text" placeholder="John" label="Leader First Name" value="{{ old('fname') }}" required="1" />

		<x-fields::input class="col-span-3" id="lname" type="text" placeholder="Doe" label="Leader Last Name" value="{{ old('lname') }}" required="1" />
		...
		<x-fields::input class="col-span-3" id="email" type="email" placeholder="john@email.com" label="Leader Email" value="{{ old('email') }}" required="1" />

		<x-fields::input class="col-span-3" id="cemail" type="email" placeholder="john@email.com" label="Confirm Leader Email" value="{{ old('cemail') }}" required="1" />
